...
!   inquire( file=trim(newDirPath)//'/.', exist=dirExists )  ! Works with gfortran, but not ifort
      ...
      ! Intel
      inquire (directory=newDirPath, exist=dirExists)         ! Works with ifort/ifx, but not gfortran
#elif __GFORTRAN__ == 1
      ! Gfortran
      inquire (file=newDirPath, exist=dirExists)         ! Works with gfortran, but not ifort/ifx
#else
      ! Other
      dirExists = .false. ! unknown
#endif
      if (dirExists) then
!      write (*,*) "Directory already exists: '"//trim(newDirPath)//"'"
   ...
   subroutine disp_core(X, n1, n2, n3, title, uid)
      ...
      character(len=*), intent(in) :: title
      real(kind=wp), intent(in) :: X(n1, n2, *)
      ! local
      integer :: i1, i2, i3
